...
This can be used to get the indices of a list, tuple or string. In Python, the indexing for a list starts at `0` and ends at one less than its length. For example, for the list, `[1, 2, 3, 4]`, the indices are `0, 1, 2, 3`. Thus, by the very nature of `range`, you can use it to get the indices of such sequences.
...
**Since it accepts integers, it can even be used with negative numbers**:
