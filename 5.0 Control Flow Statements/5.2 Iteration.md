...
Since the sequence should start at `2`, the _start_ argument is `2`. We want even numbers from `2`. If we start counting from `2`, every even number will be 2 more than the previous even number. For example, the next even number, `4` is `2 + 2` and the one after that, `6` is `4 + 2`. Therefore, the _step_ argument is `2`.
